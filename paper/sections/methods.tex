...
\end{equation}
%
where $i$ is the class index, $C$ the number of classes, and $F1_i$ defined as:
%
\begin{equation}
...
Imputing categorical columns can be seen as a classification task. Accordingly, we measure performance in this case as well as for downstream classification tasks by the $macroF1$-score. In the following, we use the terms $F1$-score and $F1$ synonymously for $macro F1$-score. For regression tasks and when imputing numerical columns, we use the $RMSE$ metric. Since, on the one hand, $F1$ is a score measure, larger values imply better performance. On the other hand, $RMSE$ is an error measure, where a smaller value indicates better performance.
