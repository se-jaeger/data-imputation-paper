...
%
Several generative models are successfully applied to data imputation, especially variational autoencoders (VAE) (TODO: cites) and generative adversarial networks (GAN) (TODO: cites). VAEs learn to encode their input into a distribution over the latent space and decode by sampling from this distribution. On the other hand, GANs consist of two parts: a generator and a discriminator. In an adversarial process, the generator learns to generate samples that are as close as possible to the data distribution, and the discriminator learns to distinguish whether an example is true or generated.

Our VAE imputer is based on the traditional architecture \citep{VAE}. For the missing values, we insert random noise similar to \cite{CaminoVAE}. In training the model we use a batch size of 64, a maximum number of epochs of 50 but with early stopping after three rounds with no improvement. As optimizer, we choose Adam with default parameters and a learning rate of 0.001. We optimize the VAE's number of hidden layers $\in \{0, 1, 2\}$ using a 3-fold cross-validation. The size of the latent space dimension as well as the hidden units, i.e. the number of neurons in the hidden layer(s) of the encoder's and decoder's neural networks, are determined relativly to the input dimensionality. We set the latent dimension size to 20\%, the size of the first hidden layer to 50\%, and the size of the second hidden layer to 30\%.
