...
In this section, we describe our six single imputation methods. The overall goal of an imputation method is to train a model on $\vec{X}\in\R^{n\times d} = [\vec{x}_1, \vec{x}_2, ..., \vec{x}_{i-1}, \vec{x}_{i+1}, ..., \vec{x}_n]$, where $d$ is the number of features, $n$ the number of observations, and $\vec{x}_i$ denotes the to-be-imputed (or target) column.
To abstract crucial steps such as encode, normalize, and decode the data and cross-validate hyperparameters of each imputation method, we define a framework implemented by all of the following imputation approaches.

\sebastian{
Will rework this section and
\begin{itemize}
	\item take care of change "imputer" into something else
	\item add Table with HPs, probably at the end of Section 3.4 to have an overview
	\item write preprocessing section and change the others accordingly
\end{itemize}
}
