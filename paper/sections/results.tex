...
\sebastian{Kürzer, vieles davon ist ja auch schon erläutert. - heterogeneous data + random sampled columns machts schwer zu vergleichen. Therefore, or each experiemental setting (table 6), we calculate rank for imputers based on mean performacne for the 3 repetitions. Because 6 imptuer, 6 ranks. 1 best, 6 worse. Methods that did not work get 6. 2 equal both get same smaller rank}
...
\begin{figure}\centering
    ...
    \caption[Imputation Ranks - Fully Observed]{\sebastian{Knackiger, vll so: Imputation quality of the six imputaiton methods trained on complete data. The imputation mehtod's rank is plotted against the missingness fraction. The columns of plots shows the chanigng missingness patter, the rows groups the result into categorical and numerical columns. ML methods , and discriminative DL tend to perform best. From left to right we can see more variance (bigger boxes) indicating that the results are less clear. Especially for categorical columns with MNAR pattern with higher missingness fractions, mean/mode seems to be a good alternative.}

    	In this figure, we visualize the results of \textit{Experiment 1} (imputation quality) in the \textit{Scenario 1} setting (training on complete data). The imputation rank is plotted against the missingness fraction. The plot is divided into six sub-plots, with three columns determined by the missingness type, ordered by task difficulty from easiest to most difficult (MCAR, MAR, MNAR). The first row shows imputation performance for categorical missing values,  the second row shows imputation performance for numerical columns. Within a single sub-plot, each box represents the distribution of ranks of each imputation method over all corresponding datasets/columns. Classical ML methods and discriminative DL tend to perform best. For categorical columns, however, there is often no improvement over Mode imputation, especially in the MNAR setting.
	}
	\label{fig:fully_observed_impute_rank_boxplot}
\end{figure}

\autoref{fig:fully_observed_impute_rank_boxplot} shows the aggregated ranks for imputation performance when imputation methods are trained on fully observed data. \sebastian{Hier muss bevor alles andere kommt eine erläuterung des Plots:
\begin{itemize}
	\item Warum boxplots? -> Sehen wir wichtige statistics die sonsts sehr überlappend wären. 25, median, 75 quantile, die eben ganz schön auf die distribution deuten.
	\item Von Links nach recht ist immer leicht zu schwerer: gilt für plot columns (MCAR, MAR, MNAR) und fractions (small to big)
	\item Weil teils deutliche Unterscheide trennten wir categorical und numerical columns -> Hier wäre es dann acuh ganz gut zu sagen, wieviele cat. und num. columns es ware..
\end{itemize}
Da fällt mir ein, vll zieht man die erläterung, dass und warum wir boxplots nehmen ganz an anfang von der resulsts section. Glaub das ist besser..
}
...
\begin{figure}\centering
    ...
    \caption[Imputation Ranks - Corrupted]{\sebastian{Wie oben: Erster Satz viel klarer, die "structur" des plots sollte knackiger beschrieben sein und dann gern oben genau das selbe}Results of \textit{Experiment 1} (imputation quality) in the \textit{Scenario 2} setting (training on incomplete/imputed data). The imputation rank is plotted against the missingness fraction. The plot is divided into six sub-plots, with three columns determined by the missingness type, ordered by task difficulty from easiest to most difficult (MCAR, MAR, MNAR). The first row shows imputation performance for categorical missing values,  the second row shows imputation performance for numerical columns. Within a single sub-plot, each box represents the distribution of ranks of each imputation method over all corresponding datasets/columns. Classical ML methods and discriminative DL tend to perform best. For categorical columns, however, there is often no improvement over Mode imputation\sebastian{Das kann man so nicht sagen.. Wir wissen nur dass es im mittel so ist..}, especially in the MNAR setting.
    }
