...
Another branch in the statistics literature focuses on multiple imputation (MI)~\citep{Rubin_MI} that has become state-of-the-art. In MI applications, one replaces missing values with multiple predictions from an imputation model. Those $M > 1$ completed data sets represent the distributions of the missing values holding more information than single estimates, which can be beneficial for downstream usage. The most popular and widely used MI technique is multiple imputation by chained equations (MICE)~\citep{Little, vanBuuren2018}. MICE is flexible and can be implemented with different models that influence its imputation performance.
